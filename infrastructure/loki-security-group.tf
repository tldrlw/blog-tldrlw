...
# By specifying the source security group IDs for the Lambda function (aws_security_group.lambda_sg.id) and the Grafana ECS container (module.ecs_service_grafana.ecs_security_group_id), you are explicitly restricting access to Loki on port 3100 to these two trusted sources only.
