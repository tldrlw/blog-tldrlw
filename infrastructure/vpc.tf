...
# You have configured three public subnets with map_public_ip_on_launch = true, meaning that any resources (such as EC2 instances) launched in these subnets will automatically receive a public IP address and have direct internet access through the internet gateway. However, for Lambda functions that run inside a VPC, placing them in public subnets is not sufficient to provide outbound internet access.
# When a Lambda function is configured to run inside a VPC, it uses Elastic Network Interfaces (ENIs) that do not get public IP addresses, even if they are associated with public subnets. Therefore, to provide internet access to Lambda functions running in a VPC, you typically need:
# Configure private subnets (without direct internet access) for the Lambda function.
